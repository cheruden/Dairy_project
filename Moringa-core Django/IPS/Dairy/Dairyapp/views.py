...
from Dairyapp.forms import AddVendorForm, MilkCategoryForm,vendorledgerForm,ProfileForm,SignUpForm,CustomerMilkCategoryForm,contactForm
...
def home(request):
    ...
    if form.is_valid():
        receivers_list = ['cheruden25@gmail.com']
...
def my_profile(request):
    ...
    profile =models.Profile.objects.get(username=current_user)
    return render(request,'profile/user_profile.html',{"profile":profile})

@login_required(login_url='/accounts/login/')
def my_profile(request):
    current_user=request.user
    records=models.Vendor.objects.filter(id=current_user.id).all()
    return render(request,'profile/user_profile.html',{"record":records})
...
def update_profile(request):
    current_user=request.user
    if request.method=="POST":
        instance = models.Profile.objects.get(username=current_user)
    ...
    elif models.Profile.objects.get(username=current_user):
        profile = models.Profile.objects.get(username=current_user)
...
def my_profile(request):
    current_user=request.user
    records = models.Dailyrecords.objects.filter(username_id=current_user.id).all()
...
def notifications(request):
    current_user=request.user
    profile=models.Profile.objects.get(address=current_user)
    all_notifications = models.Notifications.objects.all()
